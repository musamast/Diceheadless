...
class DiceScraper():
            ...
            for urldata in self.urlsData:
                ...
                if start != -1:
                    for i in range(start, len(url)):
                        if url[i] == '&':
                            url = url.replace(
                                url[start:i], 'pageSize='+str(jobsCount))
                            break
                self.driver.get(url)
                time.sleep(7)
                ...
                for i in self.driver.find_elements_by_tag_name('dhi-search-card'):
                    ...
                    jobslink.append(i.find_elements_by_tag_name('a')[
                                    0].get_attribute('href'))

                self.scrapejobs(jobslink, skillname, url,
                                importantwords, removejobwords, candiname)
    ...
    def scrapejobs(self, jobs, skillname, url, importantwords, removejobwords, candiname):
        try:
            ...
            sheet.append(['Position No', 'Candidate Name', 'Opportunity Name', 'Job Url', 'Position Id', 'Location', 'Posting time',
                          'Keywords', 'Account Name', 'Company Dice URL', 'Phone No', 'Email', 'Posted By', 'Important lines', 'Websites In JD', 'Company Details', 'Lead Source'])
            wb.save(filename)
            ...
            for job in jobs:
                ...
                self.driver.execute_script(
                    "window.open('%s', 'new_window')" % job)
                self.driver.switch_to.window(self.driver.window_handles[1])

                try:
                    positionid = self.driver.find_element_by_css_selector(
                        '.company-header-info').text.split('\n')[1]
                    Position_Id = positionid.split(':')[-1].replace(' ', '')
                    postedtime = self.driver.find_element_by_css_selector(
                        '.company-header-info').text.split('\n')[2]
                ...
                try:
                    ...
                    if Position_Id in pastpostionid:
                        ...
                        print(
                            f'Jobs ignored because of star - {self.starjobsignored}')
                        self.driver.close()
                ...
                Postion_no = count+1
                try:
                    jobdesc = self.driver.find_element_by_class_name(
                ...
                if (self.checkjob(jobdesc, removejobwords)):
                    ...
                    print(
                        f'Jobs ignored because of remove words - {self.jobsignoredbyremovewords}')
                    self.driver.close()
                    self.driver.switch_to.window(self.driver.window_handles[0])
                    continue

                try:
                    Job_title = self.driver.find_element_by_class_name(
                        'jobTitle').text
                except Exception as e:
                    pass
                ...
                try:
                    Keywords = ','.join(
                        [i.text for i in self.driver.find_elements_by_css_selector('.row.job-info')])
                except Exception as e:
                    pass

                try:
                    Company_Name = self.driver.find_element_by_id(
                        'hiringOrganizationName').text
                except Exception as e:
                    pass

                try:
                    Company_web_URL = self.driver.find_element_by_css_selector(
                        '.employer.hiringOrganization').find_element_by_tag_name('a').get_attribute('href')
                except:
                    pass
                ...
                try:
                    Posted_by_details = self.driver.find_element_by_id(
                        'contact-container').text.replace('\n', '').replace('Posted By', '')
                except:
                    pass

                try:
                    Important_lines = self.checkimportantlines(
                        jobdesc, importantwords)
                except:
                    pass
                ...
                try:
                    if (Phone_no != '' or Email != ''):
                        ...
                        if not (os.path.exists('DiceScraperFiles/VendorDetails.xlsx')):
                            wb = Workbook()
                            sheet = wb.active
                            sheet.append(['Company Name', 'Company Dice URL',
                                         'Posted by details', 'Address', 'Email', 'Phone no'])
                            sheet.append(
                                [Company_Name, Company_web_URL, Posted_by_details, Address, Email, Phone_no])
    #                         wb.save('D:/DiceScraper/DiceScraperFiles/VendorDetails.xlsx')
                            ...
                            wb = load_workbook(
                                'DiceScraperFiles/VendorDetails.xlsx')
                            sheet = wb.active
                            sheet.append(
                                [Company_Name, Company_web_URL, Posted_by_details, Address, Email, Phone_no])
        #                     wb.save('D:/DiceScraper/DiceScraperFiles/VendorDetails.xlsx')
                ...
                try:
                    Company_details_from_text_file = self.getcompdetailsfromfile(
                        Company_Name)
                except:
                    pass
            ...
            try:
                ...
                folder = drive.ListFile(
                    {'q': f"title='DiceScraperFiles' and trashed=false and mimeType='application/vnd.google-apps.folder'"}).GetList()[0]
                file = drive.CreateFile(
                    {'title': f'{filename[17:]}', 'parents': [{'id': folder['id']}]})
    ...
    def getphonenum(self, data):
        ...
        matches = []
        try:
            matches.append(self.driver.find_element_by_id(
                'contactTT').get_attribute('data-original-title'))
        except:
            pass
        i = 0
        flag = False
    ...
    def getcompdetailsfromfile(self, compname):
        if self.vendorDetailsFile != 0:
            compname = compname[:15].lower().replace(
                ' ', '').replace(',', '').replace('.', '')
